...
static ScoreParams paramsForN(void) {
    /*
     * Tuned for "10 random runs, take the best weighted total".
     * Small N keeps eating pressure; mid/high N switches to anti-trap scoring
     * because high-score tails come from long survival after the board gets crowded.
     */
    if (growN <= 1) {
        return (ScoreParams){4, 1, 18, 2400, 8500, 18};
    }
    if (growN <= 2) {
        return (ScoreParams){7, 3, 24, 3200, 13000, 24};
    }
    if (growN <= 4) {
        return (ScoreParams){4, 1, 18, 2400, 8500, 18};
    }
    if (growN <= 8) {
        return (ScoreParams){6, 5, 18, 2200, 8000, 18};
    }
    if (growN <= 16) {
        return (ScoreParams){7, 3, 24, 3200, 13000, 24};
    }
    if (growN <= 32) {
        return (ScoreParams){6, 0, 18, 2200, 8000, 18};
    }
    if (growN <= 64) {
        return (ScoreParams){6, 5, 18, 2200, 8000, 18};
    }
    if (growN <= 128) {
        return (ScoreParams){3, 0, 16, 2000, 7500, 16};
    }
    if (growN <= 256) {
        return (ScoreParams){6, 5, 18, 2200, 8000, 18};
    }
    return (ScoreParams){10, 3, 16, 1800, 6000, 14};
}

static int evaluatePathState(const GameState *s, int pathLen) {
    Point tail = s->snake[s->len - 1];
    ScoreParams p = paramsForN();
    int space = reachableSpace(s, 1);
    int canReachTail = (bfsDistance(s, tail, 1, NULL) >= 0);
    int trapped = (space < s->len + p.trapMargin);
    return 10000 - p.pathPenalty * pathLen + p.spaceWeight * space + p.tailWeight * canReachTail - p.trapPenalty * trapped;
}
